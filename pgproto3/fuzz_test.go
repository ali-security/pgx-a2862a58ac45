...
func FuzzFunctionCall(f *testing.F) {
	...
	f.Add([]byte{})
	// Truncated messages. Every read must be bounds checked against the
	// remaining message instead of trusting the declared counts and lengths.
	f.Add([]byte{0, 0, 0, 1, 0, 5, 0, 0})
	f.Add([]byte{0, 0, 0, 1, 0, 0, 0, 0})
	f.Add([]byte{0, 0, 0, 1, 0, 0, 0, 1, 0, 0})
	f.Add([]byte{0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 5, 0, 1})
	f.Fuzz(func(t *testing.T, data []byte) {
...
func FuzzSASLInitialResponse(f *testing.F) {
	...
	f.Add([]byte{})
	// The mechanism is terminated but the data size is truncated.
	f.Add([]byte("SCRAM-SHA-256\x00"))
	f.Add([]byte("SCRAM-SHA-256\x00\x00\x00"))
	f.Fuzz(func(t *testing.T, data []byte) {
...
func FuzzCopyFail(f *testing.F) {
	f.Add([]byte("something went wrong\x00"))
	f.Add([]byte{0})
	f.Add([]byte{})
	f.Fuzz(func(t *testing.T, data []byte) {
		var msg pgproto3.CopyFail
		msg.Decode(data) // must not panic
	})
}
